...
describe("The test environment", () => {
  ...
  describe("[BUG#3] A player that gets into prison always stays there; he should be able to leave.", () => {
    it("should getting out of prison", function () {
      ...
      // ASSERT
      expect(console.Content).toContain("[round 1] Rémi was sent to the penalty box");
      expect(console.Content).toContain("[round 2] Rémi is getting out of the penalty");
    });
  });
  ...
  describe("[FEATURE#1] Rock questions can be replaced by Techno questions before the game begins.", () => {
    ...
    it("should test rock question by default", function () {
      ...
      // ACT
      GameRunner.main(
        new GameBuilder().withPlayers([new Player("Rémi"), new Player("Florian"), new Player("Hugo"), new Player("Gauthier"), new Player("Nicolas"), new Player("Théo")]).withCustomConsole(console).withCoinGoal(30).build()
      );
  ...
  describe("[FEATURE#2] Every player can use one Joker per game instead of answering the question. No Gold earned.", () => {
    it("should a player can use a joker if he has one left", function () {
      ...
      // ACT
      GameRunner.main(
        new GameBuilder().withPlayers([new Player("Rémi"), new Player("Gauthier"), new Player("Florian"), new Player("Hugo")]).withCustomConsole(console).withForceJoker().build()
      );
    ...
    it("should not earn a gold when player uses a joker", function () {
      ...
      // ASSERT
      expect(console.Content).toContain("[round 1] Rémi is the current player. Has 0 gold");
      expect(console.Content).toContain("[round 1] Rémi uses a joker");
      expect(console.Content).toContain(
  ...
  describe("[FEATURE#6] Number of Gold required to win can be parameterized before the game. Minimal value : 6.", () => {
    it("should set coin goal to 7", function () {
      ...
      // ACT
      GameRunner.main(
        new GameBuilder().withPlayers([new Player('Rémi'), new Player("Constantin")]).withFirstPlayerWithOnlyTrueAnswer().withNeverUseJoker().withCoinGoal(7).withCustomConsole(console).build()
      );
